...
import imageTest from '../../assets/quartzoRosa.jpeg';
  ...
  return (
    <li className='product-card-wrapper'>
        <div className='product-card-image-wrapper'>
          <img className='product-card-image' src={imageTest} alt={`Product Card Image ${item.title}`} />
        </div>
        <h2 className='title'>{item.title.toUpperCase()}</h2>
        {item.subtitle && (
            <h3>{item.subtitle.toUpperCase()}</h3>
        )}
        ...
        <Button label={cardLabel} clickAction={() => handleAddToCart(item)} />
    </li>
  );
};